...
Each input char frame carries:
  - value (U8):   raw byte value
  - label (str):  field name assigned by the C++ LLA (e.g. "TS(direct)", "TA1(Fi=372,Di=1)")
  - from  (str):  "card" or "reader" (unreliable for T0/T1; HLA determines sender from context)
  - phase (str):  "atr", "pps", "t0", or "t1"
"""
...
class Hla(HighLevelAnalyzer):
    ...
    def __init__(self):
        ...
        # PPS state machine (reader sends first, card echoes back)
        self._pps_state = 'PPSS'
        self._pps_start = None
        self._pps_pps0 = 0
        self._pps_fi = None
        self._pps_di = None
        self._pps_exchange = 0  # 0 = reader → card, 1 = card → reader

        # T=0 state machine
        # Sender is determined from protocol position (not from char 'from' field)
        ...
        self._t0_p3 = 0
        self._t0_sw1 = 0
        self._t0_sw1_time = None
        self._t0_apdu_start = None

        # T=1 state machine
        self._t1_state = 'NAD'
        self._t1_start = None
        self._t1_block_sender = 'reader'  # alternates per block
        self._t1_pcb = 0
        self._t1_len = 0
        self._t1_data_count = 0
        self._t1_edc_len = 1   # LRC default; updated from ATR when possible
    ...
    def decode(self, frame: AnalyzerFrame):
        ...
        # Detect phase transition: flush buffered ATR when leaving 'atr'
        if phase != self._phase:
            if self._phase == 'atr' and self._atr_chars:
                atr_frame = self._emit_atr()
                if atr_frame:
                    results.append(atr_frame)
            self._phase = phase

        if phase == 'atr':
            self._collect_atr(frame, value, label)
        elif phase == 'pps':
            r = self._process_pps(frame, value)
            if r:
                results.extend(r if isinstance(r, list) else [r])
        elif phase == 't0':
            r = self._process_t0(frame, value)
            if r:
                results.extend(r if isinstance(r, list) else [r])
        elif phase == 't1':
            r = self._process_t1(frame, value)
            if r:
                results.extend(r if isinstance(r, list) else [r])
    ...
    def _collect_atr(self, frame, value, label):
        if not self._atr_chars:
            ...
            # When a new ATR starts reset T1 state to prepare for next session
            self._t1_state = 'NAD'
    ...
    def _emit_atr(self):
        ...
        # Build a summary description from the char labels assigned by the C++ LLA.
        # The TS label carries convention info; TA1 label carries Fi/Di.
        convention = ''
        fi_di = ''
        protocol = ''
        ...
        for _v, lbl in self._atr_chars:
            ...
            if 'TA1' in lbl:
                ...
                if m:
                    fi_di = f' Fi={m.group(1)} Di={m.group(2)}'
                    fi = int(m.group(1))
                    di = int(m.group(2))
                    if di > 0:
                        fi_di += f' ETU={fi // di}'

        # Detect T=1 from presence of TD bytes indicating protocol 1
        # The C++ labels the relevant bytes; look for "T=1" hint in labels
        for _v, lbl in self._atr_chars:
            if 'TD' in lbl:
                # The value isn't decoded into the label directly,
                # but if the C++ ATR node set T=1 we see it via protocol transition
                pass
        ...
        if fi_di:
            parts.append(fi_di.strip())
        if not parts:
            parts.append(f'{len(self._atr_chars)} bytes')
        ...
        self._atr_start = None
        self._atr_end = None
        # Reset PPS state for this session
        self._pps_state = 'PPSS'
    ...
    def _process_pps(self, frame, value):
        ...
        if self._pps_state == 'PPSS':
        ...
        elif self._pps_state == 'PPS1':
            fi_idx = (value >> 4) & 0x0F
            di_idx = value & 0x0F
            self._pps_fi = _get_fn(fi_idx)
            self._pps_di = _get_dn(di_idx)
        ...
        elif self._pps_state == 'PPS2':
            if self._pps_pps0 & 0x40:
                self._pps_state = 'PPS3'
            else:
                self._pps_state = 'PCK'

        elif self._pps_state == 'PPS3':
            self._pps_state = 'PCK'

        elif self._pps_state == 'PCK':
            ...
            results.append(AnalyzerFrame('pps', self._pps_start, frame.end_time,
                                         {'description': ' '.join(parts), 'sender': sender}))
            # Toggle: reader sends first, card echoes
            self._pps_exchange = 1 - self._pps_exchange
            if self._pps_exchange == 0:
                # Both sides done; reset for potential re-negotiation
                self._pps_state = 'PPSS'
            else:
                # Waiting for card echo
                self._pps_state = 'PPSS'

        return results

    # ── T=0 ─────────────────────────────────────────────────────────────────
    # Sender is determined by position: reader sends 5-byte header and
    # optional command data; card sends procedure bytes, response data, SW.
    ...
    def _process_t0(self, frame, value):
        ...
        if state == 'HEADER':
            if not self._t0_header:
                ...
                self._t0_apdu_start = frame.start_time
                self._t0_data_count = 0
            ...
            ins = self._t0_ins
            if proc == 0x60:
                # NULL — card stalling, stay in procedure
                pass
            elif (proc & 0xF0) in (0x60, 0x90):
                self._t0_sw1 = proc
                self._t0_sw1_time = frame.start_time
                self._t0_state = 'SW2'
            elif proc == ins or proc == (ins ^ 0x01):
                # ACK: all remaining bytes follow
                self._t0_data_count = 0
                self._t0_state = 'Bytes'
            elif proc == (ins ^ 0xFF) or proc == (ins ^ 0xFE):
                # ACK: one byte follows
                self._t0_state = 'Byte'
            # else: unexpected — remain in procedure

        elif state == 'SW2':
            sw_str = _sw_string(self._t0_sw1, value)
            ins_name = _ins_name(self._t0_ins)
            cla = self._t0_header[0]
            p1, p2 = self._t0_header[2], self._t0_header[3]
            tpdu_desc = f'{ins_name} CLA={cla:#04x} P1={p1:#04x} P2={p2:#04x} → {sw_str}'
            results.append(AnalyzerFrame('tpdu', self._t0_start, frame.end_time,
                                         {'description': tpdu_desc, 'sender': 'reader'}))
            results.append(AnalyzerFrame('apdu', self._t0_apdu_start, frame.end_time,
    ...
    def _process_t1(self, frame, value):
        ...
        if state == 'NAD':
            ...
            self._t1_data_count = 0
            self._t1_edc_count = 0
            # Start/continue APDU chain on I-blocks
            if (self._t1_pcb & 0x80) == 0:
            ...
            if self._t1_edc_count >= self._t1_edc_len:
                ...
                if (pcb & 0x80) == 0:
                    ...
                    if m == 0 and self._t1_apdu_active:
                        # End of APDU chain
                        apdu_desc = self._build_t1_apdu_desc()
                        results.append(AnalyzerFrame('t1_apdu', self._t1_apdu_start, frame.end_time,
                                                     {'description': apdu_desc}))
                        self._t1_apdu_active = False
                        self._t1_apdu_data = []
                    ...
                    code_str = names.get(code, f'{code:#04x}')
                    desc = f'S({code_str},{"resp" if resp else "req"})'
                    if self._t1_len > 0 and self._t1_apdu_data:
                        desc += f' val={self._t1_apdu_data[0]:#04x}'
                    results.append(AnalyzerFrame('t1_block', self._t1_start, frame.end_time,
                                                 {'description': desc, 'sender': sender}))

                # Alternate sender for next block
    ...
    def _build_t1_apdu_desc(self):
        ...
        if len(data) >= 4:
            ...
            sw_str = _sw_string(sw1, sw2)
            # Heuristic: first byte of command is CLA (not 6x/9x range)
