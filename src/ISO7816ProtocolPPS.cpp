...
ISO7816ProtocolPPS::ISO7816ProtocolPPS( ISO7816Analyzer* analyzer ) : ISO7816ProtocolLayer( analyzer ), mNode( NULL )
{
    initTransaction();
}
...
{
    if( ( mAnalyzer->GetContext()->GetSender() == sender_reader ) && ( mStatePPS == statePPS_PPSS ) && ( data == PPS_PPSS_VALUE ) )
    {
        return true;
    }
