...
static const U8 msb2lsb[] = {
    
};
...
ISO7816SimulationDataGenerator::ISO7816SimulationDataGenerator()
    : mSettings( nullptr ),
      ...
      mSimulationDataVCC( nullptr ),
      mSimulationDataRST( nullptr ),
      mSimulationDataCLK( nullptr ),
      mSimulationDataIO( nullptr )
{
}
...
void ISO7816SimulationDataGenerator::Initialize( U32 simulation_sample_rate, ISO7816AnalyzerSettings* settings )
{
    ...
    mSimulationDataVCC = mSimulationChannelsISO7816.Add( mSettings->mChannelVCC, simulation_sample_rate, BIT_LOW );
    mSimulationDataRST = mSimulationChannelsISO7816.Add( mSettings->mChannelRST, simulation_sample_rate, BIT_LOW );
    mSimulationDataCLK = mSimulationChannelsISO7816.Add( mSettings->mChannelCLK, simulation_sample_rate, BIT_LOW );
    mSimulationDataIO = mSimulationChannelsISO7816.Add( mSettings->mChannelIO, simulation_sample_rate, BIT_LOW );
}

U32 ISO7816SimulationDataGenerator::GenerateSimulationData( U64 largest_sample_requested, U32 sample_rate,
...
void ISO7816SimulationDataGenerator::GenerateSessionT0DirectPPS()
{
    ...
    for( U8 i = 0; i < sizeof( atr ); i++ )
        GenerateCharDirect( atr[ i ] );
    GenerateEtu( 150 );

    // PPS request (reader -> card): FF 10 94 7B
    static const U8 pps[] = { 0xFF, 0x10, 0x94, 0x7B };
    for( U8 i = 0; i < sizeof( pps ); i++ )
        GenerateCharDirect( pps[ i ] );
    GenerateEtu( 80 );
    ...
    // Reader: 00 A4 04 00 05 | Card: ACK=A4 | Reader: A0 00 00 00 03 | Card: 61 17
    {
        ...
        for( U8 i = 0; i < sizeof( hdr ); i++ )
            GenerateCharDirect( hdr[ i ] );
        GenerateEtu( 5 );
        GenerateCharDirect( 0xA4 ); // ACK = INS
        for( U8 i = 0; i < sizeof( data ); i++ )
            GenerateCharDirect( data[ i ] );
        GenerateEtu( 5 );
    ...
    // Reader: 00 C0 00 00 17 | Card: ACK=C0, 23 bytes FCI, 90 00
    {
        ...
        static const U8 fci[] = { 0x6F, 0x15, 0x84, 0x08, 0xA0, 0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00,
                                  0xA5, 0x09, 0x88, 0x01, 0x05, 0x5F, 0x2D, 0x02, 0x65, 0x6E, 0x00 };
        for( U8 i = 0; i < sizeof( hdr ); i++ )
            GenerateCharDirect( hdr[ i ] );
        GenerateEtu( 5 );
        GenerateCharDirect( 0xC0 ); // ACK = INS
        for( U8 i = 0; i < sizeof( fci ); i++ )
            GenerateCharDirect( fci[ i ] );
        GenerateEtu( 5 );
    ...
    // Reader: 00 20 00 80 08 | Card: ACK=20 | Reader: 8 PIN bytes | Card: 90 00
    {
        ...
        for( U8 i = 0; i < sizeof( pin ); i++ )
            GenerateCharDirect( pin[ i ] );
        GenerateEtu( 5 );
        GenerateCharDirect( 0x90 );
        GenerateCharDirect( 0x00 );
    }
...
void ISO7816SimulationDataGenerator::GenerateSessionT0MinimalNoPPS()
{
    ...
    // Reader: 00 88 00 00 08 | Card: ACK=88 | Reader: 8-byte challenge | Card: 61 08
    {
        ...
        for( U8 i = 0; i < sizeof( challenge ); i++ )
            GenerateCharDirect( challenge[ i ] );
        GenerateEtu( 5 );
    ...
    // Reader: 00 C0 00 00 08 | Card: ACK=C0, 8 response bytes, 90 00
    {
        ...
        GenerateEtu( 5 );
        GenerateCharDirect( 0xC0 ); // ACK = INS
        for( U8 i = 0; i < sizeof( resp ); i++ )
            GenerateCharDirect( resp[ i ] );
        GenerateEtu( 5 );
        GenerateCharDirect( 0x90 );
        GenerateCharDirect( 0x00 );
    }
...
void ISO7816SimulationDataGenerator::GenerateSessionT1DirectPPS()
{
    ...
    // S(IFS) request from card: NAD=00 PCB=C1 LEN=01 IFS=FE LRC=3E
    {
        ...
        for( U8 i = 0; i < sizeof( blk ); i++ )
            GenerateCharDirect( blk[ i ] );
    }
    GenerateEtu( 20 );
    ...
    {
        static const U8 blk[] = { 0x00, 0x00, 0x0A, 0x00, 0xA4, 0x04, 0x00, 0x05, 0xA0, 0x00, 0x00, 0x00, 0x03, 0x0C };
        for( U8 i = 0; i < sizeof( blk ); i++ )
            GenerateCharDirect( blk[ i ] );
    }
    GenerateEtu( 20 );
    ...
    {
        static const U8 blk[] = { 0x00, 0x60, 0x08, 0x00, 0xA4, 0x04, 0x00, 0x08, 0x01, 0x02, 0x03, 0xC0 };
        for( U8 i = 0; i < sizeof( blk ); i++ )
            GenerateCharDirect( blk[ i ] );
    }
    GenerateEtu( 20 );
    ...
    {
        static const U8 blk[] = { 0x00, 0x00, 0x05, 0x04, 0x05, 0x06, 0x07, 0x08, 0x0D };
        for( U8 i = 0; i < sizeof( blk ); i++ )
            GenerateCharDirect( blk[ i ] );
    }
    GenerateEtu( 20 );
...
void ISO7816SimulationDataGenerator::GenerateSessionT0Inverse()
{
    ...
    // Reader: 00 B0 00 00 04 | Card: ACK=B0, 4 bytes, 90 00
    {
        ...
        for( U8 i = 0; i < sizeof( hdr ); i++ )
            GenerateCharInverse( hdr[ i ] );
        GenerateEtu( 5 );
        GenerateCharInverse( 0xB0 ); // ACK
        for( U8 i = 0; i < sizeof( data ); i++ )
            GenerateCharInverse( data[ i ] );
        GenerateEtu( 5 );
