...
ISO7816ProtocolATR::ISO7816ProtocolATR( ISO7816Analyzer* analyzer ) : ISO7816ProtocolLayer( analyzer ), mNode( NULL )
{
    initTransaction();
}
...
void ISO7816ProtocolATR::newData( ISO7816Node* node )
{
    ...
    case stateATR_TB:
        snprintf( charDesc, sizeof( charDesc ), "TB%d", mNb_T );
        charNode->AddDescription( charDesc );
    ...
    case stateATR_TC:
        snprintf( charDesc, sizeof( charDesc ), "TC%d", mNb_T );
        charNode->AddDescription( charDesc );
    ...
    case stateATR_TD:
        snprintf( charDesc, sizeof( charDesc ), "TD%d", mNb_T );
        charNode->AddDescription( charDesc );
...
U8 ISO7816ProtocolATR::GetT1SpecificEDC( void )
{
    ...
    {
        /* For TD defining T1 */
        if( ( atr.T[ i ][ ATR_INTERFACE_C ].present ) && ( ( atr.T[ i - 1 ][ ATR_INTERFACE_D ].value & 0x0F ) == SC_PROTOCOL_T1 ) )
...
U8 ISO7816ProtocolATR::GetT1SpecificIFS( void )
{
    ...
    {
        /* For TD defining T1 */
        if( ( atr.T[ i ][ ATR_INTERFACE_A ].present ) && ( ( atr.T[ i - 1 ][ ATR_INTERFACE_D ].value & 0x0F ) == SC_PROTOCOL_T1 ) )
        {
...
U8 ISO7816ProtocolATR::GetT1SpecificCWI( void )
{
    ...
    {
        /* For TD defining T1 */
        if( ( atr.T[ i ][ ATR_INTERFACE_B ].present ) && ( ( atr.T[ i - 1 ][ ATR_INTERFACE_D ].value & 0x0F ) == SC_PROTOCOL_T1 ) )
